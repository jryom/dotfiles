...
start_caffeinate_for() { local secs="$1"; [ "$secs" -gt 0 ] && caffeinate -i -t "$secs" & }
is_locked() { ioreg -n Root -d1 | grep -m1 -q 'CGSSessionScreenIsLocked'; }
idle_seconds() {
  local ns
...
remaining_seconds() { local end="$1"; echo $(( end - $(now_ts) )); }
ensure_swiftbar() { pgrep -x SwiftBar >/dev/null 2>&1 || open -ga SwiftBar >/dev/null 2>&1 || true; }

status_output() {
  local end rem
  end=$(read_end)
  [[ "$end" =~ ^[0-9]+$ ]] || { echo disabled; return 1; }
  rem=$(remaining_seconds "$end")
  [ "$rem" -le 0 ] && { echo disabled; return 1; }
...
parse_input_to_epoch() {
  ...
  if [[ "$input" =~ ^[0-9]+$ ]] && [ "$input" -gt 0 ]; then
    ...
    epoch=$(date -j -f "%Y-%m-%d %H:%M" "$(date +%F) $input" +%s 2>/dev/null) || { notify "$INVALID_INPUT_MSG"; return 1; }
    now=$(now_ts)
    [ "$epoch" -le "$now" ] && epoch=$(date -j -f "%Y-%m-%d %H:%M" "$(date -v+1d +%F) $input" +%s)
  else
    notify "$INVALID_INPUT_MSG"; return 1
  fi
  echo "$epoch"
}
...
start_until_epoch() {
  ...
  local now
  now="$(now_ts)"
  [ "$epoch" -gt "$now" ] || { notify "Time already passed."; return 1; }
  stop_caffeinate
...
[ -f "$STATE_FILE" ] && is_locked && disable
